...
export class Stage extends StageBase<InitStateType, ChatStateType, MessageStateType, ConfigType> {
    ...
    buildBoard(): string {
        ...
        let result = `---\n`;
        let lines = fen.split('/');
        result += `<style>.chessboard {padding: 10px; background-color: lightcoral}div.box {width: 50px;height: 50px; display: flex; align-items: center; justify-content: center; font-size: 32px; font-family: monospace; color: darkseagreen;}.row{display: flex;} div.row:nth-child(odd).box:nth-child(odd),div.row:nth-child(even).box:nth-child(even){background: #666666;} div.row:nth-child(even).box:nth-child(odd),div.row:nth-child(odd).box:nth-child(even){background: transparent;}</style>`;
